run_permuted_multiple_regressions_fc_par <- function(brain_behave_data, electrodes, regressor, nBins, region_name, niter = 1000, sub, tag) {
  ...
  for(elec in electrodes) {
    ...
    for (bin in nBins) {
      ...
      if(grepl("bin", bin)){
        reg_1_vec <- factor(reg_1_vec, ref == "low")
        reg_2_vec <- factor(reg_2_vec, ref == "low")
      } else {
        reg_1_vec <- factor(reg_1_vec, ref == "mid")
        reg_2_vec <- factor(reg_2_vec, ref == "mid")        
      }
      ...
      beta_3[bin] <- model$coefficients[4, 1]
      beta_4[bin] <- model$coefficients[5, 1]
      beta_5[bin] <- model$coefficients[6, 1]
      beta_6[bin] <- model$coefficients[7, 1]
      lm_pval_1[bin] <- model$coefficients[2,4]
      lm_pval_2[bin] <- model$coefficients[3,4]
      lm_pval_3[bin] <- model$coefficients[4,4]
      lm_pval_4[bin] <- model$coefficients[5,4]
      lm_pval_5[bin] <- model$coefficients[6,4]
      lm_pval_6[bin] <- model$coefficients[7,4]}
    
    ## Find out the longest stretch in which pval < 0.05 and create sum-of-F-stats statistic ##
    stretch <- c(lm_pval_1 < 0.05, lm_pval_2 < 0.05, lm_pval_3 < 0.05, lm_pval_4 < 0.05, lm_pval_5 < 0.05, lm_pval_6 < 0.05)
    ...
    write.csv(results, path(here(), "results", sub, "insula", paste0(region_name, "_", elec, "_", reg_1, "_", reg_2, "_", tag, "_results.csv")))
