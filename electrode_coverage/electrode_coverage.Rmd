...
Subjects: IR9  IR10  IR14  IR16  IR17  IR19  IR26  IR28  IR35  IR39  IR57 ST40 GP51 CP34 DA7 DA8
...
* IR35 -done
* IR39 -done
* IR57 -- missing behavior, waiting on Ignacio
* ST40 -- 
...
for (sub in subjects_irvine) {
    ...
    if (sub == "IR9") {
      ...
      original <- readxl::read_xlsx('~/Projects/knight_server/remote/ecog/DATA_FOLDER/Irvine/IR26/Notes/IR26_Elec_Notes(old).xlsx')
      colnames(original)[2] <- "Loc Meeting"
      temp_notes <- merge.data.frame(temp_notes, original, by.x = "Electrode", by.y = "contact")
      temp_notes <- temp_notes %>% select(-`Loc Meeting.x`) %>% rename(`Loc Meeting` = `Loc Meeting.y`)
      temp_notes <- temp_notes[is.na(temp_notes$`Epileptic via Irvine`), ] # we want only blanks here, same as below
...
# CP34 #
cp34_notes <- readxl::read_xlsx(path(here(), "munge", "CP34_Elec_Notes.xlsx"))
cp34_notes$`Loc Meeting` <- cp34_notes$AAL # using AAL since it is a grid case
cp34_notes$Discard <- 0
cp34_notes$`Out of Brain` <- 0
cp34_notes$Epileptic <- 0
cp34_notes$subject <- "CP34"


# DA8 #
DA8_notes <- readxl::read_xlsx("/Users/bstavel/Projects/knight_server/remote/ecog/DATA_FOLDER/Davis/DA8/Notes/DA8_Elec_Notes_localized.xlsx")
DA8_notes$`Loc Meeting` <- DA8_notes$AAL # using AAL since it is a grid case
DA8_notes$Discard <- 0
DA8_notes$`Out of Brain` <- 0
DA8_notes$Epileptic <- 0
DA8_notes$subject <- "DA8"

# GP51 #
## missing data ##

# merge other data centers #
elec_notes <- bind_rows(elec_notes, st40_notes, cp34_notes)
