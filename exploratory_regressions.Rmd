...
# necessary paths #
file_path_to_electrode_names <- path(here(), "munge", "IR35_electrodes_choice_locked_cut.csv")
file_path_to_hg_data <- path(here(), "munge", "IR35_hg_munge_choice_locked_cut.csv")
...
run_filtered_anova(regression_results, hg_behave,  region_name = "All")
...
regression_results %>%
  filter(electrode == "RTH6-RTH7")  %>%
  kable() %>%
  add_header_above(c("Advantageous Inequity" = length(regression_results))) %>%
  kable_styling(position = "center")
...
dis_results_for_plots %>%
  ...
  geom_col(alpha = .5) +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), panel.background = element_rect(fill = "white")) + 
  scale_fill_manual(values = c("grey", getPalette(3)[2:3])) +
  scale_color_manual(values = c("grey", getPalette(3)[2:3])) +
  labs(y = "Difference in Adj. R Squared", x = "Bins", fill = "Region", color = "Region") +
  facet_wrap(~electrode) + 
