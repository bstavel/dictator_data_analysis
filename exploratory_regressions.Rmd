...
hg_eda_summary %>%
  ...
  filter(electrodes == "RIN3-RIN4") %>%
  ggplot(., aes(x = bin, y = mean, color = electrodes, fill = electrodes)) +
  geom_point(alpha = .4) +
  geom_line(alpha = .4) +
  # geom_vline(xintercept = 1.2613816 * 1000) +
  theme(panel.background = element_rect(fill = "white")) + 
  scale_fill_manual(values = getPalette(9)[8]) +
  scale_color_manual(values = getPalette(9)[8]) +
  ylim(-0.05, .2) +
  labs(y = "high gamma") +
  ggtitle("Insula")
...
# load data #
hg_data_full <- load_high_gamma_data(file_path_to_hg_data, file_path_to_electrode_names, "V952", "V953")
elecs_to_use <- read.csv(file_path_to_elecs_of_interest)
...
adv_plot <- hg_raw_plot %>%
  ...
  scale_fill_viridis(limits = c(-1, 3.5))   +
  scale_color_viridis(limits = c(-1, 3.5)) +
  ggtitle(paste0(elec, " -- Advantageous"))
 
dis_plot <- hg_raw_plot %>%
  mutate(advent = if_else(ineq_disadvent > 0, "Advantageous", "Disadvantageous")) %>%
  ...
  scale_fill_viridis(limits = c(-1, 3.5))   +
  scale_color_viridis(limits = c(-1, 3.5)) +
  ggtitle(paste0(elec, " -- Disadvantageous"))

  ggsave(filename = paste0('./figures/', elec, "_adv.png"),
     device = "png",
     width = 10,
     height = 10,
     units = "cm",
    ...
    ggsave(filename = paste0('./figures/', elec, "_dis.png"),
     device = "png",
     width = 10,
     height = 10,
     units = "cm",
...
presentation_results <- compile_results(regions_to_combine, type = "pres-locked")
...
run_filtered_anova(presentation_results, hg_behave,  region_name = "All", all_results = T, type = "pres-locked")

# read back in the results #
region_name <- "All"
adv_pres_results_for_plots <- read.csv(fs::path(here(), "results", paste0(region_name, "_pres-locked_anova_results_advantageous.csv")))
dis_pres_results_for_plots <- read.csv(fs::path(here(), "results", paste0(region_name, "_pres-locked_anova_results_disadvantageous.csv")))
```
