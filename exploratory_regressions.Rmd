...
nBins <- colnames(hg_clean %>% select(starts_with("bin_")))
hg_behave <- merge.data.frame(hg_clean, behave_data, by.x = "trial", by.y = "round", all.x = T, all.y = F)
write.csv(hg_behave, path(here(), "munge", "hg_behave.csv"), row.names = F)
...
## run regressions ##
# adv ineq #
run_permuted_regressions(brain_behave_data, electrodes = ofc_electrodes, regressor = "ineq_advent", nBins, region_name = "OFC", niter = 10000)
# disadv ineq #
run_permuted_regressions(brain_behave_data, electrodes = ofc_electrodes, nBins, region_name = "OFC", niter = 10000)
...
run_permuted_regressions(brain_behave_data, electrodes = insula_electrodes, nBins, region_name = "Insula", niter = 10000)
...
run_permuted_regressions(brain_behave_data, electrodes = mfg_electrodes, nBins, region_name = "MFG", niter = 10000)
...
run_permuted_regressions(brain_behave_data, electrodes = sts_electrodes, nBins, region_name = "STS", niter = 10000)


run_filtered_anova(brain_behave_data, region_name = "Insula")
run_filtered_anova(brain_behave_data, region_name = "MFG")
...
ofc_results_dis %>%
  select (-X, -X.1) %>%
  kable() %>%
  add_header_above(c("OFC: Disadvantageous Inequity" = 11)) %>%
  kable_styling(position = "center")
