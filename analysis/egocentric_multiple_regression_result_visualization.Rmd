...
betas_pres_mult_reg_results <- all_pres_mult_reg_results %>%
  select(electrode, bin, R2, Beta_1, Beta_2, Fstat, p_1, p_2, perm_p, predictor, sub, ROI)

# merge #
betas_pres_mult_reg_results <- left_join(betas_pres_mult_reg_results, prepped_pres_mult_reg_results %>% select(sub, electrode, pres_other_legend, pres_self_legend), by = c("sub", "electrode"))

# table #
tmp <- betas_pres_mult_reg_results %>% filter(predictor %in% c("ineq_disadvent_other_var_payoff", "other_var_payoff_ineq_disadvent")) %>% select(electrode, pres_other_legend, ROI) %>% distinct()
tmp_table <- data.frame(table(tmp$ROI, tmp$pres_other_legend))
colnames(tmp_table) <- c("ROI", "Pres_legend", "Count")

## ofc ##
# single regressions #
self_single_reg_ofc <- self_var_results_pres %>% 
  select(electrode, sub, perm_p,) %>%
  distinct() %>%
  rename(self_single_perm_p = perm_p)

other_single_reg_ofc <- other_var_results_pres %>% 
  select(electrode, sub, perm_p,) %>%
  distinct() %>%
  rename(other_single_perm_p = perm_p)

dis_single_reg_ofc <- ineq_disadvent_results_pres %>% 
  select(electrode, sub, perm_p,) %>%
  distinct() %>%
  rename(dis_single_perm_p = perm_p)

# multiple regressions #
ofc_df <- betas_pres_mult_reg_results %>% 
  filter(grepl("ofc", ROI, ignore.case = T) | grepl("orbit", ROI, ignore.case = T)) %>% 
  filter(perm_p < 0.05) %>%
  filter(predictor %in% c("ineq_disadvent_other_var_payoff", "other_var_payoff_ineq_disadvent", "ineq_disadvent_self_var_payoff", "self_var_payoff_ineq_disadvent")) %>%
  select(sub, electrode, ROI, perm_p, pres_other_legend, pres_self_legend) %>% 
  distinct() %>%
  arrange(pres_self_legend, pres_other_legend)

# join with single reg data #
ofc_sing_mult_df <- left_join(ofc_df, self_single_reg_ofc)
ofc_sing_mult_df <- left_join(ofc_sing_mult_df, other_single_reg_ofc)
ofc_sing_mult_df <- left_join(ofc_sing_mult_df, dis_single_reg_ofc)


ofc_self_df <- all_pres_mult_reg_results %>% filter(grepl("ofc", ROI, ignore.case = T) | grepl("orbit", ROI, ignore.case = T)) %>% filter(predictor == "ineq_disadvent_other_var_payoff") %>% select(sub, electrode, ROI, perm_p, predictor) %>% distinct()
ofc_self_elecs <- ofc_self_df %>% filter(perm_p < 0.05)

for(sub_idx in ofc_self_elecs$sub ) {
    sub_ofc_self_elecs <- ofc_self_elecs %>% filter(sub == sub_idx) %>% pull(electrode)
    for(elec in sub_ofc_self_elecs) {
        # plot #
        other_plot <- betas_pres_mult_reg_results %>%
          filter(electrode == elec & sub == sub_idx) %>%
          ...
          ggplot(., aes(x = time, y = Beta_1, color = predictor)) +
          geom_line() +
          theme(panel.background = element_rect(fill = "white"), legend.position = "top") +
          xlim(0, 1500) +
          facet_wrap(~pres_other_legend) +
          ggtitle(paste0("OFC ~ other + dis model ~", elec))
        
        self_plot <- betas_pres_mult_reg_results %>%
          filter(electrode == elec & sub == sub_idx) %>%
          ...
          ggtitle(paste0("OFC ~ self + dis model ~", elec))
...
```{r sanit-check-single-reg}

## electrodes ##
  # load #
  file_path_to_elecs_of_interest <- path(here(), "munge", paste0("IR57_elecs_of_interest.csv"))
...
## run regressions ##
# adv ineq #
run_permuted_regressions_par(brain_behave_data, electrodes = "LOF3-LOF4", regressor = "self_var_payoff", nBins, region_name =  "All", 1000, sub = "IR57", tag = "testing-single-regresion")

# modify old #
self_var_results_pres_old <- self_var_results_pres %>%
  filter(electrode == "LOF3-LOF4" & sub == "IR57") %>%
  mutate(version = "old") %>%
  select(-type, -epoch, -sub, -region, -predictor)

# read in read_results function from load_collate
self_var_results_pres_func <- pres_var_reg_df_57 %>%
  filter(electrode == "LOF3-LOF4" & sub == "IR57") %>%
  mutate(version = "func") %>%
  select(-predictor, -region)

# read in medium #
self_var_results_pres_med <- read_csv('~/Projects/dictator_analysis/dictator_game/dg_behave_analysis/results/IR57/post_eda/All_LOF3-LOF4_self_var_payoff_theta-pres-locked-hilbertRS_results.csv')
self_var_results_pres_med <- self_var_results_pres_med %>%
  mutate(version = "med") %>%
  select(-X1)

# read in new #
self_var_results_pres_new <- read_csv("../All_LOF3-LOF4_self_var_payoff_testing-single-regresion_results.csv")
self_var_results_pres_new <- self_var_results_pres_new %>%
  mutate(version = "new") %>%
  select(-X1)

# merge #
self_var_results_pres_check <- rbind(self_var_results_pres_old, self_var_results_pres_func, self_var_results_pres_med, self_var_results_pres_new)

# plot #
self_var_results_pres_check %>%
  mutate(time = 50 * as.numeric(gsub("bin_", "", bin))) %>%
  ggplot(., aes(x = time, y = Beta, color = version)) +
  geom_jitter(size = 3) +
  geom_line() +
...
Let Model 1 be:
y = B1*Dis_inequity + B2*Other_payoff, where B1 = B2

Let Model 2 be:
y = B1*Dis_inequity + B2*Other_payoff, without constraints

 
Ming suggested that if we ran anova(mode_1, model_2), then model 1 would be better? 
...
betas_pres_mult_reg_results <- read_csv(path(here(), "results", "multiple_regressions", "betas_pres_mult_reg_results.csv"))
