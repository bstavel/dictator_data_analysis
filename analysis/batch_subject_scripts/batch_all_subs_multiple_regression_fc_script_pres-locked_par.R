...
for(sub in subs){
  ...
  power_behave <- NULL
    ...
    brain_behave_data <- brain_behave_data %>%
      mutate(ineq_disadvent_fc = if_else(ineq_disadvent == 0, "low", 
                                         if_else(ineq_disadvent > 0 & ineq_disadvent < 9, "low_mid", 
                                                if_else(ineq_disadvent > 8 & ineq_disadvent < 15, "high_mid", "high" )))) %>%
      mutate(other_var_payoff_fc = if_else(other_var_payoff < 9, "low", 
                                         if_else(other_var_payoff > 8 & other_var_payoff < 13, "low_mid", 
                                                 if_else(other_var_payoff > 12 & other_var_payoff < 19, "high_mid", "high" )))) %>%
      mutate(ineq_foregone_fc =if_else(ineq_foregone < 0, "low", 
                                       if_else(ineq_foregone == 0, "mid", "high"))) %>%
      mutate(other_foregone_fc =if_else(other_foregone < 10, "low", 
                                       if_else(other_foregone == 10, "mid", "high")))
    ## run regressions ##
    
    ## allocentric vs egocentric ##
    # pres #
    run_permuted_multiple_regressions_fc_par(brain_behave_data, electrodes = all_electrodes, regressor = c("ineq_disadvent_fc", "other_var_payoff_fc"), nBins, region_name =  "All", niter, sub = sub, tag = "multiple-theta-pres-locked-hilbertRS")
    run_permuted_multiple_regressions_fc_par(brain_behave_data, electrodes = all_electrodes, regressor = c("other_var_payoff", "ineq_disadvent"), nBins, region_name =  "All", niter, sub = sub, tag = "multiple-theta-pres-locked-hilbertRS")
    # choice #
    run_permuted_multiple_regressions_fc_par(brain_behave_data, electrodes = all_electrodes, regressor = c("ineq_foregone", "other_foregone"), nBins, region_name =  "All", niter, sub = sub, tag = "multiple-theta-pres-locked-hilbertRS")
    run_permuted_multiple_regressions_fc_par(brain_behave_data, electrodes = all_electrodes, regressor = c("other_foregone", "ineq_foregone"), nBins, region_name =  "All", niter, sub = sub, tag = "multiple-theta-pres-locked-hilbertRS")
    # # adv ineq #
