...
active_data <- read_csv(path(here(),
                             "results", "active_elecs", "all_hfa_electrode_tests.csv"))

# read in active_only data #

active_only_data <- read_csv(path(here(),
                                  "results", "active_elecs", "active_hfa_electrode_tests.csv"))
...
hg_data <- read_csv(path(here(),
                         "results", "active_elecs", "combined_hfa_behave_data.csv"))
hg_choice_data <- read_csv( path(here(),
                                 "results",
                                 "active_elecs", "combined_hfa_behave_choice_data.csv"))
...
```{r visualize-by-trial-types, echo = F, fig.height=10, fig.width = 12, warning=F}
...
for(roi in rois_of_interest){
  ...
  pres_plot <-  labeled_hg_data_prepped %>%
      ...
      labs(y = "mean hfa", fill = "Trial Type", color = "Trial Type", x = "Time") +
      facet_wrap(~subject) +
