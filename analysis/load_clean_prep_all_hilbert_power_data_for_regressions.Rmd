...
load_IR57_presentation_data <- function(file_path_to_power_data, clean_name) {
  ...
  # add behavioral data, remove bad trials, remove behavioral data again #
  hg_data <- merge.data.frame(hg_data, behave_data_13, by.x = "trial", by.y = "round", all.x = T, all.y = F)
  hg_data <- hg_data %>% filter(!is.na(RT)) %>% select(trial, electrodes, starts_with("time"))
...
clean_name <- path(here(), "munge", "IR35", "delta_behave_choice_rscaler_2575_150.csv")
...
clean_name <- path(here(), "munge", "IR57", "delta_behave_choice_rscaler_2575_150.csv")
