...
motor_elecs <- elec_locations %>%
  filter(subject == "IR19") %>%
  filter(str_detect(`Loc.Meeting`, "M1|motor")) %>%
...
hg_data <- hg_data %>% filter(!is.na(RT))
...
hilbert_raw_plots <- function(elec) {
  hg_data %>%
...
lapply(plot_list, function(x) hilbert_raw_plots(x))
