...
behave_data %>%
  filter(chose_equality != "Equal") %>%
  group_by(SID, self_var_payoff) %>%
  mutate(prob_ten = sum(as.numeric(chose_equality == "10-10"))/length(chose_equality) ) %>%
  ggplot(., aes(x = self_var_payoff, y = prob_ten, color = SID)) + 
  geom_point() + 
  geom_line() +
  scale_color_manual(values = c("grey", getPalette(12))) +
  theme(panel.background = element_rect(fill = "white"))

behave_data_12 %>%
  filter(chose_equality != "Equal") %>%
  group_by(SID, other_var_payoff) %>%
  mutate(prob_ten = sum(as.numeric(chose_equality == "10-10"))/length(chose_equality) ) %>%
  ggplot(., aes(x = other_var_payoff, y = prob_ten, color = SID)) + 
  geom_point() + 
  geom_line() +
  scale_color_manual(values = c("grey", getPalette(12))) +
  theme(panel.background = element_rect(fill = "white"))
...
behave_correl_data <- behave_data_11 %>%
  select(self_payoff, other_payoff, self_var_payoff, other_var_payoff, other_foregone, self_foregone, self_diff, other_diff, norm_deviance)

behavioral_cor <- cor(behave_correl_data, use = "pairwise.complete.obs")

cor_plot <- ggcorrplot(behavioral_cor,
           lab = T,
           type = "lower",
           colors = wes_palette("Zissou1", 3, type = "continuous"))

ggsave(filename = paste0(path(here(), './figures/correlation_ind_vars.png')),
     device = "png",
     width = 10,
     height = 10,
     units = "cm",
     dpi = 200,
     plot =  print(plot(cor_plot)) )
...
## load data ##
file_path_to_hg_data <- path(here(), "munge", "IR19", "hg_behave_presentation_locked_extended_150_insula.csv")
...
## load data ##
file_path_to_hg_data <- path(here(), "munge", "IR19", "hg_behave_choice_locked_cut_fixation_150.csv")
hg_behave <- read.csv(file_path_to_hg_data)
...
rt_plot <- hg_behave %>%
  ...
  mutate(electrodes = gsub("-Ref-", "-", electrodes)) %>%
  filter(electrodes == elec & side_chosen == "Left") %>%
  arrange(desc(RT)) %>%
  mutate(trial = factor(trial, levels = trial)) %>%
  ...
  ggsave(filename = paste0('./figures/IR19/CHOICE/', elec, ".png"),
     device = "png",
     width = 10,
     height = 10,
     units = "cm",
...
behave_data_11 %>%
  ...
  ggplot(., aes(x = self_payoff, self_foregone)) +
  geom_point(alpha = .5, size = 3) +
  facet_wrap(~norm_deviance)
