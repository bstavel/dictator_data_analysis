---
title: 'August EDA 2020: Power ~ Trial Type'
...
```{r sanity-check-with-plots, eval = FALSE}
...
plots_power_envelope_by_trial_type <- function(hg_data,
  ...
  elec_name <- active_only_data %>% filter(electrodes == elec) %>% select(ROI)
  active_only_to_merge <- active_only_data %>%
    filter(electrodes == elec) %>%
    gather(key = "time", value = "significant", starts_with("time")) %>%
    mutate(time = as.numeric(gsub("time_", "", time))) %>%
    select(time, significant)
  
  elec_behave_hg <- full_join(elec_behave_hg, active_only_to_merge, by = "time") 
  ...
  if(choice == FALSE) {
      ...
      elec_behave_hg <- elec_behave_hg %>%
        mutate(time = time) %>%
        mutate(plot_sig = time) %>%
        mutate_cond((is.na(significant) | significant == TRUE), plot_sig = NA)
    ...
    if(equality == FALSE) {
      ...
      p <- elec_behave_hg %>%
        ...
        geom_rect(aes(xmin = plot_sig, xmax = plot_sig, ymin = -Inf, ymax = Inf), 
                  alpha = .0001, color = "#E8EFF7") +
        theme(panel.background = element_rect(fill = "white")) +
        geom_line() +
    ...
    if(equality == FALSE) {
      ...
      p <- elec_behave_hg %>%
        ...
        ggtitle(elec_name$ROI) +
        labs(subtitle = paste0(elec, " - shaded indicates non-signficant" ))
...
plots_power_envelope_trial_type_subsample <- function(hg_data,
  ...
  elec_behave_hg <- behave_hg_data %>% 
    ...
    mutate_cond(trial_type == "Advantageous", sample_set = sample(1:102, 102) ) %>%
    mutate_cond(trial_type == "Disadvantageous",sample_set = sample(1:102, 102) ) %>%
...
```{r IR35-get-active-elecs-choice}
...
for(elec in unique(active_only_data_35$electrodes)) {
  ...
  plots_power_envelope_by_trial_type(hg_data_35,
                                     behave_data_11,
                                     active_only_data_35,
                                     elec,
                                     choice = TRUE,
...
```{r IR57-get-active-elec-choice}

# grab electrodes
active_electrodes_IR57 <- get_active_electrodes(hg_data_57,
                                                subject = "IR57",
                                                choice = T)
...
for(elec in unique(active_only_data_57$electrodes)) {
  ...
  plots_power_envelope_by_trial_type(hg_data_57,
                                     behave_data_13,
                                     active_only_data_57,
                                     elec,
                                     choice = TRUE,
...
```{r IR19-get-active-elec-choice}

# grab electrodes
active_electrodes_IR19 <- get_active_electrodes(hg_data_19,
                                                subject = "IR19",
                                                choice = T)
...
for(elec in unique(active_only_data_19$electrodes)) {
  ...
  plots_power_envelope_by_trial_type(hg_data_19,
                                     behave_data_08,
                                     active_only_data_19,
                                     elec,
                                     choice = TRUE,
...
```{r IR39-get-active-elec-choice}

# grab electrodes
active_electrodes_IR39 <- get_active_electrodes(file_path_to_power_data,
                                  file_path_to_electrode_names,
                                  file_path_to_elecs_of_interest)


## get region names ##
active_data_39 <- merge_with_elec_info_active(active_electrodes_IR39, localizations_39)
active_only_data_39 <- active_data_39 %>% filter(active == "active")

for(elec in unique(active_only_data_39$electrodes)) {

  plots_power_envelope_by_trial_type(hg_data_39,
                                     behave_data_08,
                                     active_only_data_39,
                                     elec,
                                     choice = TRUE,
...
```{r IR28-get-active-elec-choice}

# grab electrodes
active_electrodes_IR28 <- get_active_electrodes(file_path_to_power_data, 
                                  file_path_to_electrode_names,
                                  file_path_to_elecs_of_interest)


## get region names ##
active_data_28 <- merge_with_elec_info_active(active_electrodes_IR28, localizations_28)
active_only_data_28 <- active_data_28 %>% filter(active == "active")

for(elec in unique(active_only_data_28$electrodes)) {

  plots_power_envelope_by_trial_type(hg_data_28,
                                     behave_data_10,
                                     active_only_data_28,
                                     elec,
                                     choice = TRUE,
...
```{r IR10-get-active-elec-choice}

# grab electrodes
active_electrodes_IR10 <- get_active_electrodes(file_path_to_power_data,
                                  file_path_to_electrode_names,
                                  file_path_to_elecs_of_interest)


## get region names ##
active_data_10 <- merge_with_elec_info_active(active_electrodes_IR10, localizations_10)
active_only_data_10 <- active_data_10 %>% filter(active == "active")

for(elec in unique(active_only_data_10$electrodes)) {

  plots_power_envelope_by_trial_type(hg_data_10,
                                     behave_data_04,
                                     active_only_data_10,
                                     elec,
                                     choice = TRUE,
...
```{r IR16-get-active-elec-choice}

# grab electrodes
active_electrodes_IR16 <- get_active_electrodes(file_path_to_power_data,
                                  file_path_to_electrode_names,
                                  file_path_to_elecs_of_interest)


## get region names ##
active_data_16 <- merge_with_elec_info_active(active_electrodes_IR16, localizations_16)
active_only_data_16 <- active_data_16 %>% filter(active == "active")

for(elec in unique(active_only_data_16$electrodes)) {

  plots_power_envelope_by_trial_type(hg_data_16,
                                     behave_data_06,
                                     active_only_data_16,
                                     elec,
                                     choice = TRUE,
...
```{r table-of-active-electrodes, echo = F}

active_only_data_57$sub <- "IR57"
active_only_data_35$sub <- "IR35"
active_only_data_19$sub <- "IR19"
active_only_data_39$sub <- "IR39"
active_only_data_16$sub <- "IR16"
active_only_data_28$sub <- "IR28"
active_only_data_10$sub <- "IR10"

active_only_data <- rbind(active_only_data_57, active_only_data_35, active_only_data_19,
                          active_only_data_16, active_only_data_28, active_only_data_10,
                          active_only_data_39)
active_only_data %>%
