...
prepped_pres_mult_reg_results <- all_pres_mult_reg_results %>%
  ...
  mutate(pres_results = if_else(ineq_disadvent_other_var_payoff < 0.05 & other_var_payoff_ineq_disadvent < 0.05, 3, 
                                if_else(ineq_disadvent_other_var_payoff < 0.05, 2, 
                                        if_else(other_var_payoff_ineq_disadvent < 0.05, 1, 0)))) %>%
  mutate(choice_results = if_else(ineq_foregone_other_foregone < 0.05 & other_foregone_ineq_foregone < 0.05, 3, 
                                if_else(ineq_foregone_other_foregone < 0.05, 2, 
                                        if_else(other_foregone_ineq_foregone < 0.05, 1, 0)))) %>%
  mutate(choice_legend = if_else(choice_results == 3, "Both", 
                                 if_else(choice_results == 2, "Ineq Foregone", 
                                         if_else(choice_results == 1, "Other Foregone", "Neither")))) %>%
  mutate(pres_legend = if_else(pres_results == 3, "Both", 
                                 if_else(pres_results == 2, "Ineq Disadvant", 
                                         if_else(pres_results == 1, "Other Var", "Neither"))))
...
kable(table(factor(prepped_pres_mult_reg_results$pres_legend, levels = c("Both", "Ineq Disadvant", "Other Var", "Neither"))), col.names = c("Pres Vars", "Freq"), caption = "Presentation Epoch") %>% kable_styling()
kable(table(factor(prepped_choice_mult_reg_results$choice_legend, levels = c("Both", "Ineq Foregone", "Other Foregone", "Neither"))), col.names = c("Choice Vars", "Freq"), caption = "Choice Epoch") %>% kable_styling()
kable(table(factor(prepped_pres_mult_reg_results$choice_legend, levels = c("Both", "Ineq Foregone", "Other Foregone", "Neither"))), col.names = c("Choice Vars", "Freq"), caption = "Presentation Epoch") %>% kable_styling()
kable(table(factor(prepped_choice_mult_reg_results$pres_legend, levels = c("Both", "Ineq Disadvant", "Other Var", "Neither"))), col.names = c("Pres Vars", "Freq"), caption = "Choice Epoch") %>% kable_styling()
...
betas_pres_mult_reg_plot_df <- betas_pres_mult_reg_results %>%
  group_by(sub, bin, predictor, pres_legend) %>%
  ...
  group_by(predictor, pres_legend, bin) %>%
  ...
  mutate_cond(pres_results == 2 & predictor == "other_var_payoff_ineq_disadvent", mean_beta_1 = NA, lower_sem_beta_1 = NA, upper_sem_beta_1 = NA) %>%
  mutate_cond(pres_results == 1 & predictor == "ineq_disadvent_other_var_payoff", mean_beta_1 = NA, lower_sem_beta_1 = NA, upper_sem_beta_1 = NA) %>%
  mutate(bin = as.numeric(gsub("bin_", "", bin)))

betas_choice_mult_reg_plot_df <- betas_choice_mult_reg_results %>%
  group_by(sub, bin, predictor, choice_legend) %>%
  mutate(sub_lower_beta_1_sem = mean(Beta_1, na.rm = T) - 2*(sd(Beta_1, na.rm = T)/n())) %>%
  mutate(sub_upper_beta_1_sem = mean(Beta_1, na.rm = T) + 2*(sd(Beta_1, na.rm = T)/n())) %>%
  mutate(sub_mean_beta_1 = mean(Beta_1, na.rm = T)) %>%
  group_by(predictor, choice_legend, bin) %>%
  mutate(lower_sem_beta_1 = mean(sub_mean_beta_1) - 2*(sd(sub_mean_beta_1)/n())) %>%
  mutate(upper_sem_beta_1 = mean(sub_mean_beta_1) + 2*(sd(sub_mean_beta_1)/n())) %>%
  mutate(mean_beta_1 = mean(sub_mean_beta_1)) %>%
  mutate_cond(choice_results == 2 & predictor == "ineq_foregone_other_foregone", mean_beta_1 = NA, lower_sem_beta_1 = NA, upper_sem_beta_1 = NA) %>%
  mutate_cond(choice_results == 1 & predictor == "other_foregone_ineq_foregone", mean_beta_1 = NA, lower_sem_beta_1 = NA, upper_sem_beta_1 = NA) %>%
  mutate(bin = as.numeric(gsub("post_", "", gsub("pre_", "-", bin))))


ggthemr("solarized")
betas_pres_mult_reg_plot_df %>%
  ...
  xlim(0, 30) +
  facet_wrap(~pres_legend) +
  ggtitle("All Regions ~ Pres Epoch ~ Pres Variables")


ggthemr("light")
betas_choice_mult_reg_plot_df %>%
  ...
  theme(panel.background = element_rect(fill = "white")) +
  xlim(-15, 20) +
  facet_wrap(~choice_legend) +
  ggtitle("All Regions ~ Choice Epoch ~ Choice Variables")

```


```{r sub-regions, warning=F}

roi_betas_pres_mult_reg_plot_df <- betas_pres_mult_reg_results %>%
  filter(predictor %in% c("ineq_disadvent_other_var_payoff", "other_var_payoff_ineq_disadvent")) %>%
  ...
  filter(exists == 1) %>%
  group_by(sub, bin, predictor, pres_legend, region) %>%
  mutate(sub_lower_beta_1_sem = mean(Beta_1, na.rm = T) - 2*(sd(Beta_1, na.rm = T)/n())) %>%
  mutate(sub_upper_beta_1_sem = mean(Beta_1, na.rm = T) + 2*(sd(Beta_1, na.rm = T)/n())) %>%
  mutate(sub_mean_beta_1 = mean(Beta_1, na.rm = T)) %>%
  group_by(predictor, pres_legend, bin, region) %>%
  mutate(lower_sem_beta_1 = mean(sub_mean_beta_1) - 2*(sd(sub_mean_beta_1)/n())) %>%
  mutate(upper_sem_beta_1 = mean(sub_mean_beta_1) + 2*(sd(sub_mean_beta_1)/n())) %>%
  mutate(mean_beta_1 = mean(sub_mean_beta_1)) %>%
  mutate_cond(pres_results == 2 & predictor == "other_var_payoff_ineq_disadvent", mean_beta_1 = NA, lower_sem_beta_1 = NA, upper_sem_beta_1 = NA) %>%
