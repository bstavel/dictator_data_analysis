...
respFilter <- respFilter %>%
  ...
  mutate(less_than_one = if_else(RT < 1, "Under 1s", "Above 1s")) %>%
  mutate(less_than_75 = if_else(RT < .75, "Under .75s", "Above .75s")) %>%
  mutate(less_than_5 = if_else(RT < .5, "Under .5s", "Above .5s"))
...
time_matrix <- respFilter %>%
  filter(!duplicated(min_rt)) %>%
  mutate(bins_200 = min_rt/.200) %>%
  mutate(bins_100 = min_rt/.100) %>%
  mutate(bins_50 = min_rt/.05) %>%
  select(-RT, -less_than_one, -less_than_5, -less_than_75)

kable(time_matrix) %>% kable_styling(position = "center") %>% add_header_above(c("Number of Bins pre-choice based on min RT" = 6))
  

kable(table(respFilter$SID, respFilter$less_than_one)) %>% kable_styling(position = "center")

kable(table(respFilter$SID, respFilter$less_than_75)) %>% kable_styling(position = "center")

kable(table(respFilter$SID, respFilter$less_than_5)) %>% kable_styling(position = "center")
