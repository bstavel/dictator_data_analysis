...
# load behav files #
filenames <- dir(path(here(), "../dg_behave_formatted/"),"behav.csv")
DATA <- NULL

for (i in 1:length(filenames)) {
  temp <- cbind(SID=substr(filenames[i],1,6),read.csv(path(here(), "../dg_behave_formatted/", filenames[i]), header=TRUE))
  temp$RT <- as.numeric(as.character(temp$RT))
  DATA <- rbind(DATA,temp)
}

# load task structure files #
trialTypes <- read.csv(path(here(), '../../trial_type.csv'), header = T)
